...
export async function sendPasswordResetEmail(email: string, otpCode: string) {
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
    console.error('⚠️ EMAIL_USER or EMAIL_PASS not found in .env. Password reset email was not sent natively.');
    console.log(`[SIMULATION] Sending Password Reset OTP: ${otpCode} to ${email}`);
  }

  const mailOptions = {
    from: `"رادار الذهب" <${process.env.EMAIL_USER}>`,
    to: email,
    subject: `إعادة تعيين كلمة المرور - رادار الذهب: ${otpCode}`,
    html: `
      <!DOCTYPE html>
      <html dir="rtl" lang="ar">
      <head>
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <p>
              مرحباً بك.<br>
              لقد تلقينا طلباً لإعادة تعيين كلمة المرور الخاصة بحسابك في منصة <strong>رادار الذهب</strong>.
              يرجى استخدام الرمز السري أدناه لإكمال العملية:
            </p>
            ...
            <p style="font-size: 14px; margin-bottom: 0;">
              هذا الرمز صالح لمدة <strong>15 دقيقة</strong> فقط.<br>
              إذا لم تقم بطلب إعادة التعيين، يرجى تجاهل هذه الرسالة، فحسابك آمن ولن تتأثر بياناتك.
            </p>
          </div>
