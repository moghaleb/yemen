                    ...
                    <table className="w-full text-right">
                        ...
                        <tbody className="divide-y">
                            {requests.map((req) => {
                                ...
                                const userName = req.user?.name || "مستخدم غير معروف";
                                const userEmail = req.user?.email || "البريد غير متوفر";
                                const userInitial = userName.charAt(0) || "U";
                                const requestDate = req.createdAt ? new Date(req.createdAt).toLocaleString('ar-EG') : "تاريخ غير معروف";
                                    ...
                                    <tr key={req.id}>
                                        ...
                                        <td className="p-4">
                                            <span className={`px-2 py-1 rounded text-xs font-bold ${req.requestedTier === 'VIP' ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700'
                                                ...
                                                {req.requestedTier || "باقة غير محددة"}
                                            </span>
                                        </td>
