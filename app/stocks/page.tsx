...
import { ArrowRight, Newspaper, TrendingUp, BookOpen, AlertCircle, BarChart3 } from "lucide-react";
...
export default async function StocksDashboard() {
    ...
    // 1. Fetch Analysis (EducationalContent where type=ANALYSIS & category=STOCK)
    const analysis = await prisma.educationalContent.findMany({
        where: { type: 'ANALYSIS', category: 'STOCK' },
        orderBy: { createdAt: 'desc' },
        take: 5
    });

    // 2. Fetch Active Stock Recommendations
    const recommendations = await prisma.recommendation.findMany({
        where: { type: 'STOCK', status: 'ACTIVE' },
        orderBy: { createdAt: 'desc' }
    });

    // 3. Fetch Stock News (Gated Fetch)
    ...
    // 4. Fetch Stock Articles
    const articles = await prisma.educationalContent.findMany({
        where: { type: 'ARTICLE', category: 'STOCK' },
        orderBy: { createdAt: 'desc' },
        take: 5
    });
    ...
    return (
        <div className="min-h-screen bg-slate-50 pb-20">
            {/* Header */}
            <header className="bg-cyan-600 text-white p-6 shadow-md relative overflow-hidden">
                ...
                <div className="container relative z-10">
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <h1 className="text-3xl font-bold">لوحة الأسهم (Stocks)</h1>
                    </div>
                    <p className="opacity-90 max-w-xl">
                        وجهتك الشاملة لسوق الأسهم: تحليلات فنية، فرص استثمارية، وأخبار الشركات المؤثرة على السوق.
                    </p>
                </div>
            </header>
            ...
            <main className="container px-4 py-6 -mt-6 relative z-20">
                <Tabs defaultValue="analysis" className="w-full">
                    <TabsList className="grid w-full grid-cols-4 h-auto p-1 bg-white rounded-xl shadow-sm mb-6">
                        <TabsTrigger value="analysis" className="flex flex-col gap-1 py-3 data-[state=active]:bg-cyan-100 data-[state=active]:text-cyan-800">
                            <BarChart3 className="w-5 h-5" />
                            <span className="text-xs font-bold">التحليل</span>
                        </TabsTrigger>
                        <TabsTrigger value="recommendations" className="flex flex-col gap-1 py-3 data-[state=active]:bg-cyan-100 data-[state=active]:text-cyan-800">
                            <AlertCircle className="w-5 h-5" />
                            <span className="text-xs font-bold">التوصيات</span>
                        </TabsTrigger>
                        <TabsTrigger value="news" className="flex flex-col gap-1 py-3 data-[state=active]:bg-cyan-100 data-[state=active]:text-cyan-800">
                            <Newspaper className="w-5 h-5" />
                            <span className="text-xs font-bold">الأخبار</span>
                        </TabsTrigger>
                        <TabsTrigger value="articles" className="flex flex-col gap-1 py-3 data-[state=active]:bg-cyan-100 data-[state=active]:text-cyan-800">
                    ...
                    <TabsContent value="analysis" className="space-y-4">
                        {analysis.length > 0 ? (
                            <div className="grid gap-4">
                                {analysis.map((item) => (
                                    <Link href={`/education/articles/${item.id}`} key={item.id} className="block group">
                                        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100 group-hover:border-cyan-200 group-hover:shadow-md transition-all cursor-pointer">
                                            <h3 className="font-bold text-lg text-slate-800 mb-2 group-hover:text-cyan-700">{item.title}</h3>
                            ...
                            <div className="text-center py-12 bg-white rounded-xl border border-dashed text-slate-400">
                                لا يوجد تحليلات للأسهم مضافة حديثاً.
                            </div>
                        )}
                    </TabsContent>
                    ...
                    <TabsContent value="recommendations" className="space-y-4">
                        {recommendations.length > 0 ? (
                            recommendations.map((rec) => (
                                <RecommendationCard
                                    key={rec.id}
                                    asset={rec.type === 'STOCK' ? 'سهم' : 'ذهب'} // Dynamic or forced 'سهم'
                            ...
                            <div className="text-center py-12 bg-white rounded-xl border border-dashed text-slate-400">
                                لا توجد توصيات أسهم نشطة حالياً.
                            </div>
                        )}
                    </TabsContent>
                        ...
                        {news.length > 0 ? (
                            news.map((item) => (
                                <Link href={`/news/${item.id}`} key={item.id} className="block group">
                                    <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 group-hover:border-cyan-200 group-hover:shadow-md transition-all cursor-pointer flex gap-4">
                                        <div className={`shrink-0 w-1 rounded-full ${item.impact === 'HIGH' ? 'bg-red-500' : item.impact === 'MEDIUM' ? 'bg-orange-500' : 'bg-green-500'
                                            }`}></div>
                                        <div>
                                            <h3 className="font-bold text-slate-800 mb-1 group-hover:text-cyan-700">{item.title}</h3>
                            ...
                            <div className="text-center py-12 bg-white rounded-xl border border-dashed text-slate-400">
                                لا توجد أخبار أسهم متاحة لاشتراكك الحالي.
                            </div>
                        )}
                    </TabsContent>
                    ...
                    <TabsContent value="articles" className="space-y-4">
                        {articles.length > 0 ? (
                                ...
                                <Link href={`/education/articles/${item.id}`} key={item.id} className="block group">
                                    <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100 group-hover:border-cyan-200 group-hover:shadow-md transition-all cursor-pointer">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <h3 className="font-bold text-lg text-slate-800 group-hover:text-cyan-700">{item.title}</h3>
                                        </div>
                                        <div className="prose prose-sm text-slate-600 mb-3 line-clamp-3">
                                            {item.summary}
                                        </div>
                            ...
                            <div className="text-center py-12 bg-white rounded-xl border border-dashed text-slate-400">
                                لا توجد مقالات تعليمية عن الأسهم حالياً.
                            </div>
                        )}
                    </TabsContent>
